...
class FileReader:
    ...
    def read_the_file_in_chunks(self, file_name, chunk_bounds):
        """
        Opening the file inT each thread so as to access the file
        handle in a thread-safe manner. This is very inefficient
        in real life, but in AWS S3, there is support for accessing
        ranges of files in a thread-safe manner. This application is
        just a local replica of code that works in S3 for demo purpose
        """
        file_name = file_name
        f = open(file_name, "r")
